...
const main = async () => {
  ...
  console.log("Contract deployed to: ", jankenContract.address);

  /*
   * コントラクトの残高を取得（0.1ETH）であることを確認
   */
  let contractBalance = await hre.ethers.provider.getBalance(
    jankenContract.address
  );
  console.log(
    "Contract balance:",
    hre.ethers.utils.formatEther(contractBalance)
  ...
  const jankenTxn = await jankenContract.janken("gu");
  await jankenTxn.wait();

  /*
   * コントラクトの残高を取得し、Waveを取得した後の結果を出力
   */
  contractBalance = await hre.ethers.provider.getBalance(jankenContract.address);
  /*
   *コントラクトの残高から0.0001ETH引かれていることを確認
   */
  console.log(
  ...
  let allJankens = await jankenContract.getAllJankens();
  console.log(allJankens);
};
